...
use crate::dune_views::{Translated, ViewOutcome};
...
pub fn emit(dir: &Path, source: &str) -> Result<BTreeMap<String, String>> {
    ...
    render(
        ...
        &schema.tables,
        &sem,
        &crate::analytics::nest_view_files(dir),
    )
}
...
pub fn render(
    ...
    for outcome in crate::dune_views::translate(views, tables, source)? {
        let ViewOutcome {
            file,
            statement,
            name,
        ...
        match (name, result) {
            (Some(name), Ok(view)) => {
                let model = format!(
                    "{}_{}_view_{}",
                    identifier_part("nest name", nest, true)?,
                    identifier_part("chain", chain, true)?,
                    identifier_part("view name", &name, false)?
                );
                let out = format!("{model}.sql");
                if files.contains_key(&out) {
                    bail!("view `{name}` would be emitted as `{out}`, which a table's query already uses");
                }
                let reads: Vec<String> = view
                    .reads
                    .iter()
                    .map(|t| format!("`dune.{source}.{t}`"))
                    .collect();
                emitted.insert(
                    out.clone(),
                    if reads.is_empty() {
                ...
                files.insert(out, view_query(&model, &file, &name, &view, sem));
            }
            (Some(name), Err(why)) => {
                skipped.insert(format!("`{file}` view `{name}`"), why);
            }
            (None, result) => {
                let why = result
                    .err()
                    .unwrap_or_else(|| "defines no view".to_string());
                skipped.insert(format!("`{file}` statement {statement}"), why);
            }
        }
    }
    files.insert(
...
fn view_query(model: &str, file: &str, name: &str, view: &Translated, sem: &Semantic) -> String {
    let mut out = format!(
        "-- {model}\n-- Generated by `nuthatch emit dune` from view `{name}` in `{file}` (RFC-0055 S3). \
         Regenerate; do not edit.\n"
    );
    if let Some(v) = sem.views.get(name) {
        let description = one_line(&v.description);
        if !description.is_empty() {
            out.push_str(&format!("--\n-- {description}\n"));
        }
    }
...
fn readme(
    ...
    let mut out = format!(
        "# DuneSQL queries for `{nest}` on {chain}\n\n\
         ...
         `block_number`, `log_index`, `_seq` and `block_timestamp`. Each view's file reads those same \
         tables and projects the `_dec` and `_overflow` companions as the nest does, so it answers \
         over the uploaded rows what the nest's view answers over its own, except the unsealed tip, \
         which is never uploaded. Regenerate rather than edit.\n\n\
         ## Emitted\n\n"
    );
    if emitted.is_empty() {
