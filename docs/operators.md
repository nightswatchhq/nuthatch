...
Written against **0.9.2** (2026-07-30). Read [Known gaps](#known-gaps) before exposing `/sql`.
...
docker run -d --name nuthatch --restart unless-stopped \
  ...
  ghcr.io/nightswatchhq/nuthatch:0.9.2
```
...
```sh
docker run --rm ghcr.io/nightswatchhq/nuthatch:0.9.2-scaled worker --help
```
