...
Written against **1.0.2** (2026-08-04). Read [Known gaps](#known-gaps) before exposing `/sql`.
...
docker run -d --name nuthatch --restart unless-stopped \
  ...
  ghcr.io/nightswatchhq/nuthatch:1.0.2
```
...
```sh
docker run --rm ghcr.io/nightswatchhq/nuthatch:1.0.2-scaled worker --help
```
