...
label, which is the machine-readable form of this document. This is a queue, not a plan: the reasoning
for the ordering is here, the work lives in the issues, and the issues are the source of truth for
status.
...
1. **The Linux dev box** (`/home/pepe/nuthatch`, Manjaro, GCC 15). Note every build here needs
   ...
   catch that because it pins `ubuntu-22.04`.
...
**Items 4-13 are the standing queue**, ranked on four criteria applied in order. An item satisfying an
earlier criterion outranks one satisfying only a later one.
...
| # | Issue | Why here |
|---|-------|----------|
| 1 | [#364](https://github.com/nightswatchhq/nuthatch/issues/364) Early cutoff never runs in the runtime | Slice 5 shipped the mechanism and `Manifest::data_identity()` has **exactly one caller in the codebase**: `migrate.rs:292`. So adoption happens during the one-time layout migration and the runtime's mount path never consults it. The published "~0.14s adoption on a 428 MB nest" figure is `nuthatch migrate`, not a live edit. |
...
| 5 | [#284](https://github.com/nightswatchhq/nuthatch/issues/284) Peak-RSS regression gate for the dense multi-nest scenario | The other half of the same non-negotiable, and the more damning half: the CI gate is a single nest at `--backfill 200`. The dense case was measured once, out of band, and never wired in. | 2 |
...
| 7 | [#362](https://github.com/nightswatchhq/nuthatch/issues/362) `main` has no branch protection, so `--auto` merges immediately | Minutes of work, and it guards everything else in this list. Demonstrated on 2026-08-06: `--auto` merged #360 with four checks still running. | 4 |
...
**7 is nearly free** and should probably go first in wall-clock terms even though it ranks seventh,
because it protects the merges that the other twelve will produce.
