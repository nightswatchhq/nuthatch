...
| [0022](0022-distributed-scaled-mode.md) | Distributed scaled mode - read/write planes, writer pool, dynamic placement | 0013, 0019, 0021 | **Implemented** (2026-07-30) - shipped in 0.8.0/0.8.1. All six §Testing acceptance items pass; `HotStore` trait + `PgStore`, cursor leases with a store-enforced monotonic fence, pure-placement scheduler, control plane, reconcile loop, fleet-wide version pinning, runtime secret injection. Verified beyond CI on a clean Hetzner box (level 5: **10/10, zero skipped**). **Still unverified across real machines** - partition and clock-skew cases are open (see [verification.md](../verification.md)). Slice 3b (view rebuilds onto the trait) outstanding |
