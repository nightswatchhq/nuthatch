...
| [0028](0028-adaptive-log-range-control.md) | Closing the gaps in `eth_getLogs` range control | 0004 §2, 0026 | **Draft** (2026-07-28, revised) - **a fix pack, not a new capability**: §0 records that adaptive splitting/shrink-retry has existed since PR #57 and the first draft was wrong to claim otherwise. The measured gap is marker coverage - `arb1.arbitrum.io`, a **shipped default**, says `"logs matched by query exceeds limit of 10000"`, which matches none of `is_result_too_large`'s markers, so splitting never fires on our own zero-setup Arbitrum path. Fixes: wider markers + a speculative split for unclassifiable failures, honouring Alchemy's suggested range, pool-wide 429 escalation, classifier consolidation. §4 stands independently: **segment boundaries follow fetch boundaries**, so content-addressing depends on RPC tuning. Slice 1 merged (#165) |
