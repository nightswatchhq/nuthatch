...
| [0026](0026-fault-quarantine-and-partial-health.md) | Fault quarantine and partial health - a roost survives its sick nests | 0012, 0021 | **Draft** (2026-07-27) - issue #147, the blast-radius conformance gap; mini-RFC: fault taxonomy (nest vs cursor, retryable vs terminal), quarantine + backoff re-admission, live `/nests` health + roost-root `/ready`, `--fail-fast`; **slices 1-2 shipped** (a nest's error no longer kills its cursor; a cursor's death no longer kills the roost - #147's headline scenario closed and tested), slice 3 (health surface) pending |
