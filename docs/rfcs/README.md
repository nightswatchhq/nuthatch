...
| [0028](0028-adaptive-log-range-control.md) | Adaptive `eth_getLogs` range control - stop making the operator guess | 0004, 0026 | **Draft** (2026-07-28) - from ETHGlobal Pragma Lisbon 2026 feedback, where a team wrote an RPC proxy to split oversized `eth_getLogs` for us. Settles the error taxonomy (narrowable vs transient vs **terminal** - auth failures stop being retried forever), AIMD window control per endpoint with `--window` becoming a ceiling, and observability. §4 is the sting: **segment boundaries currently follow fetch boundaries**, so content-addressing already depends on the operator's `--window`/`--concurrency`; this decouples them. Also closes COR-5 |
