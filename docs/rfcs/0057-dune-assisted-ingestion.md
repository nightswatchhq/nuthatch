...
|---|---|---|
| H1 | Full-chain backfill from Dune is infeasible on Analyst: required export volume exceeds monthly allowance by ≥ 100× for every chain Nuthatch supports that Dune covers (E8). A supported chain Dune does not cover is outside H1: it is recorded as unavailable, not as confirming a volume nobody measured. | Any covered supported chain's raw logs+txs for the nest's required range fit in < 40 GB (100× the monthly allowance). |
| H2 | A contract-scoped seed backfill (all logs for a fixed contract set) fits in one month's allowance for The Graph protocol contracts on Arbitrum One, the one candidate E3 measures. A result for that nest says nothing about any other. | E3's projected total credits for that nest, query execution plus 10 per MB exported, exceed the 4,000-credit monthly allowance. |
...
| H5 | For full-chain nests, a self-hosted archive node beats Alchemy on cost per backfilled block. A Dune hybrid is not part of this comparison: for full-chain nests, H1 and G1 decide whether Dune can backfill at all, and H5 is only worth deciding if they say it cannot. | Measured cost/block for self-host ≥ Alchemy cost/block after amortizing hardware over 12 months. |
...
| G1 (after E2 and E8) | H1 confirmed for every supported chain Dune covers (E2), and every supported chain Dune does not cover recorded as unavailable (E8) | Close Q1 for full-chain nests permanently; stop entertaining "backfill from Dune" |
| G2 (after E3) | Seed for the candidate nest projected under 4,000 credits in total, execution plus export, since compute is charged as well (§2) and 400 MB of export alone is the whole allowance | S2 stays on the table for measurement; else S2 is closed for that nest, and reopens for another nest only on its own E3 measurement |
| G3 (after E4+E5) | ≥ 90 % CU reduction, < 50 credits, and no uncaught false negatives, on both E4 contract sets (the Graph candidate and the less sparse second set) | S1 accepted as the standard backfill path for log-derived scoped nests, with S1 step 4's range checksum mandatory over each nest's full range, because E5 checks hint correctness on a 100k-block sample and a sample cannot prove the rest; §8 determines whether it needs a separate RFC |
...
| 2026-09-14 | Review of #1382: H1 is scoped to the chains Dune covers. An uncovered chain is recorded as unavailable in its own right, rather than counted as confirming H1's volume threshold, which E8 cannot measure. |
| 2026-09-14 | Review of #1382: H2 and G2 gate on projected total credits, execution plus export, rather than export size alone, since 400 MB of export is already the whole 4,000-credit allowance before any compute is charged. |
| 2026-09-14 | Review of #1382: G3 no longer accepts S1 on E5's 100k-block sample alone. S1 step 4's range checksum becomes mandatory over each nest's full range, which is what catches a block the hint missed outside the sample. |
