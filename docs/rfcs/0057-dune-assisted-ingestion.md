...
| H2 | A contract-scoped seed backfill (all logs for a fixed contract set) fits in one month's allowance for The Graph protocol contracts on Arbitrum One, the one candidate E3 measures. A result for that nest says nothing about any other. | E3's estimated export size for that nest > 400 MB. |
