...
fn sparse_delegation_fold_matches_the_original_event_by_event() {
    ...
    let sql = include_str!("../examples/network/views/41-delegation.sql");
...
fn sparse_lock_fold_matches_the_original_event_by_event() {
    ...
    let sql = include_str!("../examples/network/views/45-indexer.sql");
...
fn network_startup_validator_binds_the_same_scalar_functions_as_queries() {
    let source = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("examples/network");
...
fn independently_ingested_cold_history_matches_genesis_reference() {
    ...
    for capture in [
        include_str!("../examples/network/validation/genesis-parity.json"),
        include_str!("../examples/network/validation/genesis-million-reference.json"),
        include_str!("../examples/network/validation/backfill-84m-reference.json"),
        include_str!("../examples/network/validation/backfill-123m-reference.json"),
        include_str!("../examples/network/validation/backfill-123m-indexers-reference.json"),
        include_str!("../examples/network/validation/backfill-129m-reference.json"),
        include_str!("../examples/network/validation/backfill-129m-deployments-reference.json"),
    ] {
...
fn independently_indexed_allocations_match_reference_including_arbitrum_l1_numbers() {
    ...
    let reference: Value = serde_json::from_str(include_str!(
        "../examples/network/validation/legacy-allocations-reference.json"
    ))
    .unwrap();
...
fn captured_genesis_facts_match_same_block_network_subgraph() {
    ...
    let evidence: Value = serde_json::from_str(include_str!(
        "../examples/network/validation/genesis-parity.json"
    ))
    .unwrap();
    ...
    let controls: Value = serde_json::from_str(include_str!(
        "../examples/network/validation/genesis-controls.json"
    ))
    .unwrap();
    let mut hot = hot;
...
fn issuance_follows_the_allocator_and_denylist_changes_are_historical() {
    ...
    std::fs::write(
        ...
        include_str!("../examples/network/views/30-rewards.sql"),
    )
    .unwrap();
    let mut hot = analytics::HotRows::new();
    let mut event = |table: &str, block: u64, fields: Value| {
...
fn escrow_history_matches_the_upstream_balance_signer_and_redemption_rules() {
    ...
    std::fs::write(
        ...
        include_str!("../examples/network/views/10-escrow.sql"),
    )
    .unwrap();
