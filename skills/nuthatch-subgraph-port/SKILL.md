...
deliverable. Scaffolding is RFC-0044 S2 (`#1211`) and is not this skill.
...
every entity field in `schema.graphql`, classified against the mappings.

**Do not run AssemblyScript.** Read the mappings. `nuthatch port-report --dir <subgraph>` does the
same read and prints the report; treat its classes as the starting point and investigate any field
that disagrees with a hand port.
...
# 2. A human accepts the report. Then, and only then, S2 scaffolds:
#    nuthatch init --from-subgraph <CID>
```
...
This one translates a subgraph into a report. It does not invent flags.
...
4. **Do not run the mappings.** Do not add a GraphQL compatibility layer. Do not emit nest files here.
